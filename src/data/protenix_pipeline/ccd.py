...
from src.data.protenix_pipeline.configs_data import data_configs
from src.data.protenix_pipeline.substructure_perms import get_substructure_perms

logger = logging.getLogger(__name__)

COMPONENTS_FILE = data_configs["ccd_components_file"]
RKDIT_MOL_PKL = Path(data_configs["ccd_components_rdkit_mol_file"]) if data_configs["ccd_components_rdkit_mol_file"] else None

# Lazy on-demand generation fallback
_gemmi_ccd_cif = None


def _get_gemmi_ccd_cif():
    global _gemmi_ccd_cif
    if _gemmi_ccd_cif is None:
        import gemmi
        _gemmi_ccd_cif = gemmi.cif.read(str(COMPONENTS_FILE))
    return _gemmi_ccd_cif


def _get_component_rdkit_mol_on_demand(ccd_code: str) -> Optional[Chem.Mol]:
    """Lazily generate a single RDKit mol from the CCD components.cif file."""
    try:
    ...
    # Copy atom name for pickle compatibility
    ...
    if mol.sanitized:
        options = Chem.AllChem.ETKDGv3()
        options.clearConfs = False
        try:
            conf_id = Chem.AllChem.EmbedMolecule(mol, options)
            mol.ref_conf_id = conf_id
            mol.ref_conf_type = "rdkit"
            mol.ref_mask[:] = True
        except Exception:
            logger.warning("Warning: fail to generate conf for %s, use idea conf", ccd_code)
    return mol
...
_ccd_rdkit_mols_is_complete: bool = False
...
def get_component_rdkit_mol(ccd_code: str) -> Union[Chem.Mol, None]:
    ...
    """
    global _ccd_rdkit_mols, _ccd_rdkit_mols_is_complete
    # If we have a complete cache from pickle, use it
    if _ccd_rdkit_mols_is_complete:
        return _ccd_rdkit_mols.get(ccd_code, None)

    # If this specific code is already in our partial on-demand cache, return it
    if ccd_code in _ccd_rdkit_mols:
        return _ccd_rdkit_mols[ccd_code]

    rdkit_mol_pkl = RKDIT_MOL_PKL
    if rdkit_mol_pkl is not None and rdkit_mol_pkl.exists():
        with open(rdkit_mol_pkl, "rb") as f:
            _ccd_rdkit_mols = pickle.load(f)
            _ccd_rdkit_mols_is_complete = True
        return _ccd_rdkit_mols.get(ccd_code, None)

    # Fallback: generate on demand from CCD components.cif
    if not _ccd_rdkit_mols:
        logger.warning(
            "CCD RDKit mol pickle not found at %s, falling back to on-demand generation from %s",
            rdkit_mol_pkl,
            COMPONENTS_FILE,
        )
    mol = _get_component_rdkit_mol_on_demand(ccd_code)
    if mol is not None:
        _ccd_rdkit_mols[ccd_code] = mol
    return mol
