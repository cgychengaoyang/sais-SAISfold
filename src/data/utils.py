...
move_to_np = lambda x: x.cpu().detach().float().numpy()
...
def torch_rotvec_to_quat(rotvec):
    """Rotation vector -> quaternion [w, x, y, z] (pure torch)."""
...
def torch_quat_multiply(q1, q2):
    """Quaternion composition q1 * q2 for [w, x, y, z]."""
...
def torch_rotvec_to_matrix(rotvec):
    """Rotation vector -> rotation matrix (pure torch)."""
    ...
    return (
        ...
        + (1.0 - torch.cos(theta))[..., None] * torch.matmul(K, K)
    )


def compose_rotvec(r1, r2):
    """Compose two rotation vectors on GPU with pure torch."""
