...
def _extract_trans_rots(rigid: ru.Rigid):
    ...
    quat = rigid.get_rots().get_quats()
    rotvec = du.quat_to_rotvec(quat)
    return tran, rotvec


def _assemble_rigid(rotvec, trans, device=torch.device('cpu')):
    if not torch.is_tensor(rotvec):
        rotvec = torch.tensor(rotvec, device=device).float()
    if not torch.is_tensor(trans):
        trans = torch.tensor(trans, device=device).float()
    rotvec = rotvec.to(device)
    trans = trans.to(device)
    rotmat = du.torch_rotvec_to_matrix(rotvec)
    return ru.Rigid(
        rots=ru.Rotation(rot_mats=rotmat.to(device)),
        trans=trans.to(device),
    )
...
class SE3Diffuser:
    ...
    def forward_marginal(
        ...
        t_t = t if torch.is_tensor(t) else torch.tensor(t, device=trans_0.device).view(1)
        ...
        if not self._diffuse_rot:
            ...
            rot_t, rot_score = self._so3_diffuser.forward_marginal(rot_0, t_t)
            rot_score_scaling = self._so3_diffuser.score_scaling(t_t)
        ...
        if not self._diffuse_trans:
            ...
            trans_t, trans_score = self._r3_diffuser.forward_marginal(trans_0, t_t)
            trans_score_scaling = self._r3_diffuser.score_scaling(t_t)
        ...
        rigids_t = _assemble_rigid(rot_t, trans_t, trans_0.device)
        if as_tensor_7:
    ...
    def score(
        ...
        t_t = t if torch.is_tensor(t) else torch.tensor(t, device=tran_0.device).view(1)

        if not self._diffuse_rot:
            rot_score = torch.zeros_like(rot_0)
        else:
            rot_score = self._so3_diffuser.score(rot_t, t_t)

        if not self._diffuse_trans:
            trans_score = torch.zeros_like(tran_0)
        else:
            trans_score = self._r3_diffuser.score(tran_t, tran_0, t_t, use_torch=True)
    ...
    def reverse(
        ...
        if device is None:
            device = rigid_t.get_trans().device
