...
class SO3Diffuser:
    ...
    def to_device(self, device):
        ...
        if self.gpu_initialized and self.device == device:
            return
        self.device = device
        self._score_scaling_t = torch.tensor(
            self._score_scaling, device=device, dtype=torch.float32
        )
        self._score_norms_t = torch.tensor(
            self._score_norms, device=device, dtype=torch.float32
        )
        self._discrete_omega_t = torch.tensor(
            self.discrete_omega, device=device, dtype=torch.float32
        )
        self._discrete_sigma_t = torch.tensor(
            self.discrete_sigma, device=device, dtype=torch.float32
        )
        self._cdf_t = torch.tensor(self._cdf, device=device, dtype=torch.float32)
        self._pdf_t = torch.tensor(self._pdf, device=device, dtype=torch.float32)
        self._max_sigma_t = torch.tensor(
            self.max_sigma, device=device, dtype=torch.float32
        )
        self._min_sigma_t = torch.tensor(
            self.min_sigma, device=device, dtype=torch.float32
        )
        ...
        if torch.is_tensor(t):
            ...
            if self.schedule == 'logarithmic':
                max_sigma = (
                    ...
                    else torch.tensor(self.max_sigma, device=t.device, dtype=t.dtype)
                )
                min_sigma = (
                    self._min_sigma_t
                    if self.gpu_initialized and self.device == t.device
                    else torch.tensor(self.min_sigma, device=t.device, dtype=t.dtype)
                )
                return torch.log(
                    t * torch.exp(max_sigma) + (1 - t) * torch.exp(min_sigma)
                )
        else:
            ...
            if self.schedule == 'logarithmic':
                return np.log(
                    t * np.exp(self.max_sigma) + (1 - t) * np.exp(self.min_sigma)
                )
    ...
    def diffusion_coef(self, t):
        ...
        if torch.is_tensor(t):
            ...
            return torch.sqrt(
                2
                * (torch.exp(max_sigma) - torch.exp(min_sigma))
                * sigma_t
                / torch.exp(sigma_t)
            )
        return np.sqrt(
    ...
    def sample_igso3(
        ...
        """
        if torch.is_tensor(t):
            if not self.gpu_initialized or self.device != t.device:
                self.to_device(t.device)
            t_idx = self.t_to_idx(t)
            cdf_row = self._cdf_t[t_idx]
            if cdf_row.dim() > 1:
                cdf_row = cdf_row.reshape(-1, cdf_row.shape[-1])[0]
            else:
                cdf_row = cdf_row.reshape(-1)
            u = torch.rand(n_samples, device=t.device)
            omega_idx = torch.bucketize(u, cdf_row)
            omega_idx = torch.clamp(omega_idx, 0, self._discrete_omega_t.shape[0] - 1)
    ...
    def score(
        ...
        if torch.is_tensor(vec):
            t_t = t if torch.is_tensor(t) else torch.tensor(t, device=vec.device).view(1)
            return self.torch_score(vec, t_t, eps=eps)
        if not np.isscalar(t):
            raise ValueError(f'{t} must be a scalar.')
        torch_score = self.torch_score(
            torch.as_tensor(vec, dtype=torch.float32),
            torch.tensor(t, dtype=torch.float32).view(1),
            eps=eps,
        )
        return torch_score.cpu().numpy()
    ...
    def torch_score(
        ...
        Returns:
            ...
            magnitude given by _score_norms.
        """
        if not self.gpu_initialized or self.device != vec.device:
            self.to_device(vec.device)

        omega = torch.linalg.norm(vec, dim=-1) + eps
        if self.use_cached_score:
            t_idx = self.t_to_idx(t)
            score_norms_t = self._score_norms_t[t_idx]
            if score_norms_t.dim() > 1 and score_norms_t.shape[0] == 1:
                score_norms_t = score_norms_t[0]
            omega_idx = torch.bucketize(omega, self._discrete_omega_t[:-1])
            omega_idx = torch.clamp(omega_idx, 0, self._score_norms_t.shape[-1] - 1)
            if score_norms_t.dim() >= omega_idx.dim():
                omega_scores_t = torch.gather(score_norms_t, -1, omega_idx)
            else:
                omega_scores_t = score_norms_t[omega_idx]
        else:
            sigma = self._discrete_sigma_t[self.t_to_idx(t)]
            if sigma.dim() == 0:
                sigma = sigma.unsqueeze(0)
            omega_vals = igso3_expansion(omega, sigma, use_torch=True)
            omega_scores_t = score(omega_vals, omega, sigma, use_torch=True)
    ...
    def score_scaling(self, t: np.ndarray):
        ...
        if torch.is_tensor(t):
            if not self.gpu_initialized or self.device != t.device:
                self.to_device(t.device)
            return self._score_scaling_t[self.t_to_idx(t)]
    ...
    def forward_marginal(self, rot_0: np.ndarray, t: float):
        ...
        """
        if not torch.is_tensor(rot_0):
            rot_0 = torch.as_tensor(rot_0, dtype=torch.float32)
        t_t = t if torch.is_tensor(t) else torch.tensor(t, device=rot_0.device).view(1)
        if t_t.device != rot_0.device:
            t_t = t_t.to(rot_0.device)
        n_samples = int(np.prod(rot_0.shape[:-1]))
        sampled_rots = self.sample(t_t, n_samples=n_samples)
        rot_score = self.torch_score(sampled_rots, t_t).reshape(rot_0.shape)
    ...
    def reverse(
        ...
        """
        if not self.gpu_initialized or self.device != rot_t.device:
            self.to_device(rot_t.device)
        g_t = self.diffusion_coef(t)
        perturb = (g_t ** 2) * score_t * dt + g_t * sqrt_dt * (noise_scale * z)
        if mask is not None:
            perturb *= mask[..., None]
        return du.compose_rotvec(rot_t, perturb)
