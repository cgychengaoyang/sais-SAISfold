...
from src.model.protenix.metrics.clash import Clash
from src.model.protenix.utils.distributed import traverse_and_aggregate
...
def _compute_full_data_and_summary(
    ...
    """
    # Remap token_asym_id to be contiguous 0..N-1 for crop evaluation
    token_asym_id = token_asym_id.long()
    _, inverse_indices = torch.unique(token_asym_id, return_inverse=True)
    token_asym_id = inverse_indices
...
def calculate_chain_based_gpde(
    ...
    asym_id = asym_id.long()
    unique_asym_ids, inverse_indices = torch.unique(asym_id, return_inverse=True)
    asym_id = inverse_indices
