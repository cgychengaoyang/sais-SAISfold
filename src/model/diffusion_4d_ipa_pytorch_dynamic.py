"""Fork of Openfold's IPA - Simplified for pure seq->structure folding."""
...
class InvariantPointAttention(nn.Module):
    ...
    def forward(
        ...
        v_pts_flat = v_pts.reshape(v_pts.shape[:-2] + (-1,)).transpose(-2, -3)
        o_pt_flat = torch.matmul(a, v_pts_flat.to(dtype=a.dtype))
        o_pt = o_pt_flat.reshape(o_pt_flat.shape[:-1] + (self.no_v_points, 3))
        o_pt_out_ti = o_pt.transpose(-3, -4)

        o_pt = r[..., None, None].invert_apply(o_pt_out_ti)
...
class DFOLDIpaScore(nn.Module):
    """Pure seq->structure folding model - no temporal/reference conditioning."""
    ...
    def __init__(self, model_conf, diffuser):
        ...
        for b in range(int(ipa_conf.num_blocks)):
            ...
            # Module for rigids update and edge update
    ...
    def forward(self, node_embed, edge_embed, input_feats, drop_ref=False, is_training=True):
        '''
        Pure seq->structure folding forward pass.
        
        Args:
            node_embed: [1, N, D] node features with timestep embedding
            edge_embed: [1, N, N, D_edge] edge features
            input_feats: input information from dataloader
        '''
        # Initialize the input tensors
        diffuser_time_t = input_feats['t']
        if diffuser_time_t.dim() == 0:
            diffuser_time_t = diffuser_time_t.unsqueeze(0)
        ...
        # Initialize rigids from noisy input
        ...
        # Single frame processing (no motion/ref frames)
        frame_node = node_embed
        frame_edge = edge_embed
        frame_node_init = frame_node

        # Loop of IPA blocks
        use_checkpoint = getattr(self._ipa_conf, 'gradient_checkpointing', False) and self.training
        
        for b in range(self._ipa_conf.num_blocks):
            # Run IPA with current frame only
            if use_checkpoint:
                ipa_embed = cp.checkpoint(
                    self.trunk[f'ipa_{b}'],
                    frame_node,
                    frame_edge,
                    curr_rigids,
            ...
            frame_node = self.trunk[f'ipa_ln_{b}'](frame_node + ipa_embed)

            # Sequence transformer (no spatial/temporal conditioning)
            seq_tfmr_in = torch.cat([
                frame_node, 
                self.trunk[f'skip_embed_{b}'](frame_node_init)
            ], dim=-1)
            
            if use_checkpoint:
                seq_tfmr_out = cp.checkpoint(
                    self.trunk[f'seq_tfmr_{b}'],
                    seq_tfmr_in,
                    None,  # src_mask
                    1 - node_mask,  # src_key_padding_mask
                    use_reentrant=False
                )
            else:
            ...
            frame_node = self.trunk[f'node_transition_{b}'](frame_node)

            # Update backbone rigids
            ...
            # Update edges with node features
            if b < self._ipa_conf.num_blocks - 1:
                frame_edge = self.trunk[f'edge_transition_{b}'](frame_node, frame_edge)
                frame_edge = frame_edge * edge_mask[..., None]
        ...
        # Predict angles with the last layer node features
        unorm_angles, angles = self.angle_resnet(frame_node, frame_node_init)

        # Convert rigids to tensor format for output
        rigids_tensor = curr_rigids.to_tensor_7()
        
        # Build atom37 positions from rigids and angles
        # This is needed for loss computation
        atom37_pos = None
        atom14_pos = None
        if angles is not None:
            # Get psi angles (last torsion angle)
            psi_angles = angles[..., 2, :]  # [B, N, 2]
            # Build backbone atoms from rigids and psi
            from src.data import all_atom
            atom37_pos, atom37_mask, _, _ = all_atom.compute_backbone(
                curr_rigids, psi_angles
            )
            # For now, atom14 is same as first 14 atoms of atom37
            atom14_pos = atom37_pos[:, :, :14] if atom37_pos is not None else None
