...
class RNAConverter():
    ...
    def build_cords(self, seq, fram, angl, rtn_cmsk=False):
        ...
        # Handle both 3D (no batch dim) and 4D (with batch dim) inputs
        # fram: [N, 7] or [batch, N, 7]
        # angl: [N, 6, 2] or [batch, N, 6, 2]
        original_fram_dim = fram.dim()
        if fram.dim() == 2:
            # No batch dimension: [N, 7] -> [1, N, 7]
            fram = fram.unsqueeze(0)
        if angl.dim() == 3:
            # No batch dimension: [N, 6, 2] -> [1, N, 6, 2]
            angl = angl.unsqueeze(0)
        
        # Now fram is [batch, N, 7] and angl is [batch, N, 6, 2]
        batch_size = fram.shape[0]
        n_resds = fram.shape[1]
        total_resds = batch_size * n_resds
        
        # Flatten batch and residue dimensions: [batch, N, ...] -> [batch*N, ...]
        fram = fram.reshape(total_resds, 7)
        angl = angl.reshape(total_resds, 6, 2)
        
        # Normalize angles
        angl = angl / (torch.norm(angl, dim=2, keepdim=True) + self.eps)
        
        # Convert to 4x4 transformation matrices
        rigid = Rigid.from_tensor_7(fram, normalize_quats=True)
        fram = rigid.to_tensor_4x4()
        # fram is now [total_resds, 4, 4]
        rot = fram[:,:3,:3]
        tsl = fram[:,:3,3:].permute(0,2,1)

        fram = torch.cat([rot, tsl], dim=1)[:,:4,:3].unsqueeze(1)
        fmsk = torch.ones((total_resds, 1), dtype=torch.int8, device=device)
        amsk = torch.ones((total_resds, RNA_CONSTANTS.N_ANGLS_PER_RESD_MAX), dtype=torch.int8, device=device)
        cord = torch.zeros((total_resds, RNA_CONSTANTS.ATOM_NUM_MAX, 3), dtype=torch.float32, device=device)
        cmsk = torch.zeros((total_resds, RNA_CONSTANTS.ATOM_NUM_MAX), dtype=torch.int8, device=device)
        
        # Repeat sequence for each batch element
        seq_expanded = seq * batch_size

        for resd_name in RNA_CONSTANTS.RESD_NAMES:
            idxs = [x for x in range(total_resds) if seq_expanded[x] == resd_name]
            if len(idxs) == 0:
                continue
            cord[idxs], cmsk[idxs] =\
        ...
        cord = cord.reshape(batch_size, n_resds, RNA_CONSTANTS.ATOM_NUM_MAX, 3)
        cmsk = cmsk.reshape(batch_size, n_resds, RNA_CONSTANTS.ATOM_NUM_MAX)
        
        # If original input was unbatched, remove the batch dimension from output
        if original_fram_dim == 2:
            cord = cord.squeeze(0)
            cmsk = cmsk.squeeze(0)
