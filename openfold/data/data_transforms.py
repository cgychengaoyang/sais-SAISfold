...
def make_atom14_masks(protein):
    ...
    # Pad to support RNA/DNA restype indices (up to 31)
    while len(restype_atom14_to_atom37) < 32:
        restype_atom14_to_atom37.append([0] * 14)
        restype_atom37_to_atom14.append([0] * 37)
        restype_atom14_mask.append([0.0] * 14)
    ...
    # Use 32 rows to support RNA/DNA restype indices (Protenix uses up to 31)
    restype_atom37_mask = torch.zeros(
        [32, 37], dtype=torch.float32, device=protein["aatype"].device
    )
...
def make_atom14_positions(protein):
    ...
    restype_3 += ["UNK"]
    # Pad to 32 entries for RNA/DNA compatibility
    while len(restype_3) < 32:
        restype_3.append("UNK")
    ...
    # Create an ambiguous atoms mask.  shape: (32, 14) to support RNA/DNA indices.
    restype_atom14_is_ambiguous = protein["all_atom_mask"].new_zeros((32, 14))
    for resname, swap in rc.residue_atom_renaming_swaps.items():
...
def atom37_to_frames(protein, eps=1e-8):
    ...
    restype_rigidgroup_base_atom_names = np.full([32, 8, 3], "", dtype=object)
...
def atom37_to_torsion_angles(
    ...
    # Save original aatype to zero out torsion masks for RNA/DNA later
    original_aatype = aatype
    ...
    # Zero out torsion masks for RNA/DNA residues (original aatype > 20)
    # so OpenFold transforms don't compute fake protein backbone angles for them.
    torsion_angles_mask = torsion_angles_mask * (original_aatype < 21).float().unsqueeze(-1)
